...
namespace GD.BBPH.APP.HETHONG
{
    public partial class FRM_LOGIN : FRM_DMPARENT
    {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (pass == datapass)
                    {
                        ...
                        GD.BBPH.APP.LIB.USERS._Current = (UsersEntity)_Collection[0];
                        GD.BBPH.APP.LIB.USERS._RolesEntity = (new RolesManager()).SelectOne(GD.BBPH.APP.LIB.USERS._Current.Rolesid);
                        GD.BBPH.APP.LIB.USERS._Rolesid = GD.BBPH.APP.LIB.USERS._Current.Rolesid;
                        GD.BBPH.APP.LIB.SESSION_START.TS_USER_LOGIN = user;
        ...
        public static void UpdateKey(string strKey, string newValue)
        {
            ...
            xmlDoc.Load(AppDomain.CurrentDomain.BaseDirectory + "GD.BBPH.APP.exe.config");
            if (!KeyExists(strKey))
            {
            ...
            xmlDoc.Save(AppDomain.CurrentDomain.BaseDirectory + "GD.BBPH.APP.exe.config");
