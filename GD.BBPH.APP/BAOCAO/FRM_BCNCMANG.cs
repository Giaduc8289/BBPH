...
{
    public partial class FRM_BCNCMANG : FRM_DMPARENT
    {
        ...
        private void btn_IN_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                _DataAccessAdapter.CallRetrievalStoredProcedure("Baocaonhucaumangtheodonhang", parameters, dsKetqua, 0);
                ...
                #region Hien bao cao
                new FRM_REPORTVIEWER(dsKetqua, LIB.PATH.BBPH_PATH + @"\RDLC\NHUCAUMANG.rdlc", "Ketqua", this.Text, true).Show();
                #endregion
            }
            catch (Exception ex)
            {
        ...
        private void txt_MADON_Validating(object sender, CancelEventArgs e)
        {
            ...
            string Str_MASIEUTHI = txt_MADON.Text.Trim().ToUpper();
            _RowViewSelect = checkmaDon(Str_MASIEUTHI, DT_DONHANG);
            if (_RowViewSelect == null)
            {
                ListviewJanus _frm_SingerRows_Select =
                    new ListviewJanus(LIB.PATH.BBPH_PATH + @"\XMLCONFIG\FRM_DONHANG_H.xml",
                        DT_DONHANG, DonhangHFields.Madon.Name, Str_MASIEUTHI);
        ...
        private DataRow checkmaDon(string masieuthi, DataTable dt)
        {
            try
            {
                return dt.Select(DonhangHFields.Madon.Name + "=" + "'" + masieuthi + "'").CopyToDataTable().Rows[0];
            }
            catch { return null; }
        }
