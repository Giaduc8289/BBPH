...
{
    public partial class FRM_BAOCAONHAPXUATTONKHOMANG : FRM_DMPARENT
    {
        private DataTable DT_DMKHO = new DataTable(), DT_DMMANG = new DataTable();
        private DataRow _RowViewSelect = null;
        private MenuroleEntity _MenuroleEntity = new MenuroleEntity();
        private string FUNCTION = "LOAD";
        public FRM_BAOCAONHAPXUATTONKHOMANG()
        {
            InitializeComponent();
        ...
        private void FORM_PROCESS()
        {
            ...
            using (BackgroundWorker worker = new BackgroundWorker())
            {
                ...
                worker.DoWork += delegate
                {
                    if (FUNCTION == "LOAD")
                    {
                        _MenuroleEntity = MenuroleManager.Return_Current_Menurole("FRM_NHAPXUATTONMANG");
        ...
        private void btn_IN_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                parameters[3] = new SqlParameter("@Mamang", _Mamang);
                _DataAccessAdapter.CallRetrievalStoredProcedure("Baocaonhapxuattonkhomang", parameters, dsKetqua, 0);
                ...
                #region Hien bao cao
                new FRM_REPORTVIEWER(dsKetqua, LIB.PATH.BBPH_PATH + @"\RDLC\BAOCAONHAPXUATTONKHOMANG.rdlc", "Ketqua", this.Text, true).Show();
                #endregion
            }
            catch (Exception ex)
            {
        ...
        private void txt_MAMANG_Validating(object sender, CancelEventArgs e)
        {
            ...
            _RowViewSelect = checkmaNguyenlieu(_str_MACANTIM, DT_DMMANG);
            if (_RowViewSelect == null)
            {
                ListviewJanus _frm_SingerRows_Select =
