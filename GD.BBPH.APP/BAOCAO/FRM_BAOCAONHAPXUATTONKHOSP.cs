...
{
    public partial class FRM_BAOCAONHAPXUATTONKHOSP : FRM_DMPARENT
    {
        private DataTable DT_DMKHO = new DataTable(), DM_HANG = new DataTable();
        private DataRow _RowViewSelect = null;
        private MenuroleEntity _MenuroleEntity = new MenuroleEntity();
        private string FUNCTION = "LOAD";
        public FRM_BAOCAONHAPXUATTONKHOSP()
        {
            InitializeComponent();
        ...
        private void FORM_PROCESS()
        {
            ...
            using (BackgroundWorker worker = new BackgroundWorker())
            {
                ...
                worker.DoWork += delegate
                {
                    if (FUNCTION == "LOAD")
                    {
                        _MenuroleEntity = MenuroleManager.Return_Current_Menurole("FRM_NHAPXUATTONSP");
                        ...
                        DM_HANG = new DmhangManager().SelectAllRDT();
        ...
        private void btn_IN_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                parameters[3] = new SqlParameter("@Masanpham", _Masanpham);
                _DataAccessAdapter.CallRetrievalStoredProcedure("Baocaonhapxuattonkhosanpham", parameters, dsKetqua, 0);
                ...
                #region Hien bao cao
                new FRM_REPORTVIEWER(dsKetqua, LIB.PATH.BBPH_PATH + @"\RDLC\BAOCAONHAPXUATTONKHOSP.rdlc", "Ketqua", this.Text, true).Show();
                #endregion
            }
            catch (Exception ex)
            {
        ...
        private void txt_MASANPHAM_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(txt_MASANPHAM.Text.Trim()) || DM_HANG == null || DM_HANG.Rows.Count == 0) return;
            string _str_MACANTIM = txt_MASANPHAM.Text.Trim().ToUpper();
            _RowViewSelect = checkmaMang(_str_MACANTIM, DM_HANG);
            if (_RowViewSelect == null)
            {
                ListviewJanus _frm_SingerRows_Select =
                    new ListviewJanus(LIB.PATH.BBPH_PATH + @"\XMLCONFIG\FRM_DMHANG.xml",
                        DM_HANG, DmhangFields.Masp.Name, _str_MACANTIM);
                _frm_SingerRows_Select.ShowDialog();
                if (_frm_SingerRows_Select._RowViewSelect == null) return;
                _RowViewSelect = _frm_SingerRows_Select._RowViewSelect.Row;
