...
{
    public partial class FRM_BAOCAODONHANG : FRM_DMPARENT
    {
        private DataTable DT_DMCONGNHAN = new DataTable(), DT_DMKHACH = new DataTable(), DT_DMHANG = new DataTable();
        private DataRow _RowViewSelect = null;
        private MenuroleEntity _MenuroleEntity = new MenuroleEntity();
        private string FUNCTION = "LOAD";
        public FRM_BAOCAODONHANG()
        {
            InitializeComponent();
        ...
        {
            string _Manv = "", _Makhach = "", _Masanpham = "";
            ...
            try
            {
                ...
                parameters[2] = new SqlParameter("@Manhanvien", _Manv);
                parameters[3] = new SqlParameter("@Makhach", _Makhach);
                parameters[4] = new SqlParameter("@Masanpham", _Masanpham);
                _DataAccessAdapter.CallRetrievalStoredProcedure("Baocaodonhang", parameters, dsKetqua, 0);
                ...
                #region Hien bao cao
                new FRM_REPORTVIEWER(dsKetqua, LIB.PATH.BBPH_PATH + @"\RDLC\BAOCAODONHANG.rdlc", "Ketqua", this.Text, true).Show();
                #endregion
            }
            catch (Exception ex)
            {
        ...
        private void txt_MAYIN_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(txt_MANV.Text.Trim()) || DT_DMCONGNHAN == null || DT_DMCONGNHAN.Rows.Count == 0) return;
            string Str_MASIEUTHI = txt_MANV.Text.Trim().ToUpper();
            _RowViewSelect = checkmaMay(Str_MASIEUTHI, DT_DMCONGNHAN);
            if (_RowViewSelect == null)
            {
                ...
                txt_MANV.Text = _RowViewSelect[DmcongnhanFields.Macn.Name].ToString();
                txt_TENNV.Text = _RowViewSelect[DmcongnhanFields.Tencongnhan.Name].ToString();
            }
            else
            {
                txt_TENNV.Text = _RowViewSelect[DmcongnhanFields.Tencongnhan.Name].ToString();
